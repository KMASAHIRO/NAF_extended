...
def convert_centered_npz_to_wav_and_points(
    npz_root: str,
    output_root: str,
    ...
    point_id_map = {}
    pos_id_set = set()
    tx_rx_to_pos_id = dict()  # tx_rx_index → source_target_id
    ...
    for tx_dir in sorted(npz_root.glob("tx_*"), key=lambda p: int(p.name.split("_")[1])):
        ...
        for rx_dir in sorted(tx_dir.glob("rx_*"), key=lambda p: int(p.name.split("_")[1])):
            ...
            for ir_file in ir_files:
                ...
                tx_rx_to_pos_id[f"{tx_idx}_{rx_idx}"] = pos_id  # 後でsplitに使う
