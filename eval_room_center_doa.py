...
# - Txは固定4点: (2.0,3.5), (2.0,4.5), (3.0,3.5), (3.0,4.5)
# - 各Txを中心に、半径0.5/1.0/1.5の円周上を10°刻みでRx(アレイ中心)を配置
# - dir_ch>1: Rxはアレイ中心。円形アレイ(半径 array_radius)が室内に収まる点のみ採用
# - dir_ch==1: 8本の仮想円形アレイを同中心に配置してDoA
...
def main():
    ...
    ap.add_argument("--deg_step",     type=float, default=10.0)    # 角度刻み（既定: 10°）
    args = ap.parse_args()
    ...
    # 集約バッファ
    summary_lines = ["idx,tx_x,tx_y,radius,theta_deg,used,err_deg\n"]
    ...
    if doa_ch_conf > 1:
        ...
        for tx_xy in tx_list:
            for r0 in rx_radii:
                # 0..350°をdeg_stepで
                thetas = np.arange(0.0, 360.0, args.deg_step, dtype=float)
                for th in thetas:
                    rad = math.radians(th)
                    rx_center = tx_xy + r0 * np.array([math.cos(rad), math.sin(rad)], dtype=float)

                    # アレイが室内に収まるか
                    if not array_within_bounds(rx_center, args.array_radius, min_xy, max_xy):
                        summary_lines.append(f"{sid},{tx_xy[0]},{tx_xy[1]},{r0},{th},0,NaN\n")
                        sid += 1
                        continue
                    ...
                    non_norm_tensor = torch.tensor([tx_xy[0], tx_xy[1], rx_center[0], rx_center[1]],
                    ...
                    # 真値角：Rx中心 -> Tx
                    true_deg = int(math.degrees(math.atan2(tx_xy[1] - rx_center[1], tx_xy[0] - rx_center[0])) % 360.0)
                    ...
                    all_err.append(err)
                    summary_lines.append(f"{sid},{tx_xy[0]},{tx_xy[1]},{r0},{th},1,{err:.4f}\n")
                    sid += 1
        ...
        for tx_xy in tx_list:
            for r0 in rx_radii:
                ...
                for th in thetas:
                    rad = math.radians(th)
                    center_xy = tx_xy + r0 * np.array([math.cos(rad), math.sin(rad)], dtype=float)
